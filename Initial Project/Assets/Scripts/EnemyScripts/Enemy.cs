...
public class Enemy : MonoBehaviour
{
  ...
  //  public float spaceBetween;
    ...
    public void Move(Vector2 velocity)
    {
       ...
       /* if (Vector2.Distance(goal.position, transform.position) >= eCont.neightbourRadius)
        {
            velocity = goal.position - transform.position;
            enemyAnim.SetFloat("Horizontal", velocity.x);
            enemyAnim.SetFloat("Vertical", velocity.y);
            transform.position += (Vector3)velocity * Time.deltaTime;
        }
        else
        {
            velocity = transform.position - goal.transform.position;
            enemyAnim.SetFloat("Horizontal", -velocity.x);
            enemyAnim.SetFloat("Vertical", -velocity.y);
            transform.position += (Vector3)velocity * Time.deltaTime;
        } */
